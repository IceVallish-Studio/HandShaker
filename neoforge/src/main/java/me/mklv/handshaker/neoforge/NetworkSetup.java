...
public class NetworkSetup {
    ...
    public static void registerPayloads(RegisterPayloadHandlersEvent event) {
        ...
        registrar.playToServer(
                ...
                (payload, context) -> withServer(server -> server.handleModsList(payload, context))
        );
        ...
        veltonRegistrar.playToServer(
                ...
                (payload, context) -> withServer(server -> server.handleVelton(payload, context))
        );

        LOGGER.info("All payloads registered once at mod startup");
    }

    private static void withServer(Consumer<HandShakerServerMod> consumer) {
